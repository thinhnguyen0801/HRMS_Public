...
{
    public class SalaryExpenseAccountingListController : DocumentControllerBase
    {
        ...
        #region Properties
        public List<SalaryExpenseAccountingModel>? ListPending { get; set; }
        public IGrid? GridPending { get; set; }
        public List<SalaryExpenseAccountingModel>? ListAll { get; set; }
            ...
            if (firstRender)
            {
                try
                {
                    ...
                    ListBreadcrumbs = new List<BreadcrumbModel>()
                    {
                        ...
                        new BreadcrumbModel("Danh sách hạch toán chi phí lương", isActive: true)
                    };
                    FromDate = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 01);
                    ToDate = DateTime.Now;
                    await NotifyBreadcrumb.InvokeAsync(ListBreadcrumbs);
                    await getSalaryExpenseAccounting();
                }
                catch (Exception ex)
                {
        ...
        private async Task getSalaryExpenseAccounting()
        {
            ...
            var listResult = await _salaryService.GetMasterDataAsync<SalaryExpenseAccountingModel>(request, isShowToast: true);
            listResult = listResult?.Update(m =>
            {
                ...
                m.link = "hach-toan-chi-phi-luong?key=" + _encryptHelper.Encrypt(JsonConvert.SerializeObject(pParams));
            })?.ToList();
            if (ActiveTabIndex == 0)
            {
        ...
        protected async Task ExportExcelHandler()
        {
            try
            {
                if (ActiveTabIndex == 0)
                {
                    ...
                    await ShowLoading();
                    await GridPending!.ExportToXlsxAsync("Danh-sach-hach-toan-chi-phi-luong", new GridXlExportOptions()
                    {
                ...
                await ShowLoading();
                await GridAll!.ExportToXlsxAsync("Danh-sach-hach-toan-chi-phi-luong", new GridXlExportOptions()
                {
                    ExportTotalSummaries = false,
                    ExportGroupSummaries = false
                });
