...
namespace HNOne.Web.Controllers
{
    public class PositionController : DocumentControllerBase
    {
        ...
        private async Task getPositions()
        {
            ...
            ListPosition = await _masterDataService.GetPositionAsync(UserId, Token, BranchId, $"{BranchIds}", isShowToast: true);
        }
        ...
        protected async Task ExportExcelHandler()
        {
            try
            {
                if (GridPosition == null || ListPosition.IsNullOrEmpty())
                {
                    ShowWarning(MessageConstants.MESSAGE_NOT_FOUNT);
                    return;
                }
                await ShowLoading();
                await GridPosition!.ExportToXlsxAsync("Chuc-vu", new GridXlExportOptions()
                {
