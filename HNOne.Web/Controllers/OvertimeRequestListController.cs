...
namespace HNOne.Web.Controllers
{
    public class OvertimeRequestListController : DocumentControllerBase
    {
        ...
        public List<OvertimeRequestModel>? ListAll { get; set; }
            ...
            if (firstRender)
            {
                try
                {
                    ...
                    ListBreadcrumbs = new List<BreadcrumbModel>()
                    {
                        ...
                        new BreadcrumbModel("Danh sách đề nghị làm thêm", isActive: true)
                    };
                    FromDate = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 01);
                    ToDate = DateTime.Now;
                    await NotifyBreadcrumb.InvokeAsync(ListBreadcrumbs);
                    await getOvertimeRequestList();
                }
                catch (Exception ex)
                {
        ...
        private async Task getOvertimeRequestList()
        {
            ...
            listResult = listResult?.Update(m =>
            {
                ...
                m.link = "de-nghi-lam-them?key=" + _encryptHelper.Encrypt(JsonConvert.SerializeObject(pParams));
            })?.ToList();
            if (ActiveTabIndex == 0)
            {
        ...
        protected async Task ExportExcelHandler()
        {
            try
            {
                if (ActiveTabIndex == 0)
                {
                    ...
                    await ShowLoading();
                    await GridPending!.ExportToXlsxAsync("Danh-sach-de-nghi-lam-them", new GridXlExportOptions()
                    {
                ...
                await ShowLoading();
                await GridAll!.ExportToXlsxAsync("Danh-sach-de-nghi-lam-them", new GridXlExportOptions()
                {
                    ExportTotalSummaries = false,
                    ExportGroupSummaries = false
                });
