...
namespace HNOne.Web.Controllers
{
    public class ConfirmWorkingDayListController : DocumentControllerBase
    {
        ...
        public List<ConfirmWorkingDayModel>? ListAll { get; set; }
            ...
            if (firstRender)
            {
                try
                {
                    ...
                    ListBreadcrumbs = new List<BreadcrumbModel>()
                    {
                        ...
                        new BreadcrumbModel("Danh sách xác nhận giờ công", isActive: true)
                    };
                    await NotifyBreadcrumb.InvokeAsync(ListBreadcrumbs);
                    FromDate = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 01);
                    ToDate = DateTime.Now;
                    await buildComboAsync();
                    await getConfirmWorkingDayRequestList();
                }
                catch (Exception ex)
                {
        ...
        private async Task getConfirmWorkingDayRequestList()
        {
            ...
            var listResult = await _workforceService.GetMasterDataAsync<ConfirmWorkingDayModel>(request, isShowToast: true);
            listResult = listResult?.Update(m =>
            {
                ...
                m.link = "xac-nhan-gio-cong?key=" + _encryptHelper.Encrypt(JsonConvert.SerializeObject(pParams));
            })?.ToList();
            if (ActiveTabIndex == 0)
            {
        ...
        protected async Task ExportExcelHandler()
        {
            try
            {
                if (ActiveTabIndex == 0)
                {
                    ...
                    await ShowLoading();
                    await GridPending!.ExportToXlsxAsync("Danh-sach-xac-nhan-gio-cong", new GridXlExportOptions()
                    {
                ...
                await ShowLoading();
                await GridAll!.ExportToXlsxAsync("Danh-sach-xac-nhan-gio-cong", new GridXlExportOptions()
                {
                    ExportTotalSummaries = false,
                    ExportGroupSummaries = false
                });
