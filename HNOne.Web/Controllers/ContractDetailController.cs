    ...
    public class ContractDetailController : DocumentControllerBase
    {
        ...
        private async Task buildComboAsync()
        {
            try
            {
                ...
                var getTask6 = _masterDataService.GetBranchAsync(UserId, Token, BranchId, $"{BranchIds}", supperAdmin: IsAdmin ? "Y" : "N");
        ...
        private async Task getSalaryConfigDefault()
        {
            try
            {
                var lstSalaryConfig = await _masterDataService.GetSalaryConfigAsync(UserId, Token, BranchId, opt: CommonConstants.ENUM_ACTIVE, isShowToast: false);
