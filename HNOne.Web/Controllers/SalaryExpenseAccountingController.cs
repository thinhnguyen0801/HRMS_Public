...
{
    public class SalaryExpenseAccountingController : DocumentControllerBase
    {
        ...
        public SalaryExpenseAccountingModel SalaryRequestDocument { get; set; } = new SalaryExpenseAccountingModel();
        public List<SalaryExpenseAccounting1Model>? ListSalaryExpenseAccounting { get; set; } // danh sách hạch toán chi phí lương
        public IGrid? GridSalaryExpenseAccounting { get; set; }
        ...
        private async Task buildComboAsync()
        {
            try
            {
                ...
                var getTask6 = _salaryService.GetMasterDataAsync<ComboboxModel>(request);
                await Task.WhenAll(
                    getTask3,
                    getTask4,
                    getTask5,
                ...
                ListCboAccouting = await getTask6;
        ...
        private async Task showVoucher()
        {
            try
            {
                ...
                var task1 = _salaryService.GetMasterDataAsync<SalaryExpenseAccountingModel>(request);
                var task2 = getDocumentHistory();
                await Task.WhenAll(task1, task2);
                List<SalaryExpenseAccountingModel>? lstData = await task1;
                if (!lstData.IsNullOrEmpty())
                {
                    ...
                    {
                        ListSalaryExpenseAccounting = JsonConvert.DeserializeObject<List<SalaryExpenseAccounting1Model>>(SalaryRequestDocument.jsonDetail);
                    }
                }
            }
            catch (Exception ex)
        ...
        /// <summary>
        /// lấy danh sách lương cần hạch toán
        /// </summary>
        /// <returns></returns>
        private async Task getAccountingSalaryType()
        {
            try
            {
                ListSalaryExpenseAccounting = new List<SalaryExpenseAccounting1Model>();
                if (string.IsNullOrEmpty(SalaryRequestDocument.salaryPreiod)) return;
                RequestModel request = new RequestModel();
                request.process = ProcessConstants.GET_SALARY_MASTER_DATA;
                request.userId = UserId;
                request.branchId = BranchId;
                request.token = Token;
                request.type = ProcessConstants.GET_COMBO_LIST_ACCOUNTED_SALARY_TYPE;
                request.opt = $"{SalaryRequestDocument.salaryPreiod}-01";
                var lstSalaryType = await _salaryService.GetMasterDataAsync<SalaryExpenseAccounting1Model>(request);
                ListSalaryExpenseAccounting = lstSalaryType;
            }
            catch (Exception) { throw; }
        }
        ...
        {
            if(ListSalaryExpenseAccounting.IsNullOrEmpty())
            {
                errorMessage = "Không tìm thấy danh sách chi tiết. Vui lòng làm mới danh sách chi tiết lương cần hạch toán!!!";
                fieldName = "gridInfo";
                return;
            }
        ...
        protected async Task SaveDataHandler()
        {
            try
            {
                ...
                SalaryRequestDocument.docTotal = ListSalaryExpenseAccounting!.Sum(m => m.lineTotal);
                SalaryRequestDocument.branchId = BranchId;
                ...
                string jsonDetail = JsonConvert.SerializeObject(ListSalaryExpenseAccounting!);
        ...
        protected async Task RefreshDataHandler()
        {
            try
            {
                ...
                _navigationManager.NavigateTo($"/hach-toan-chi-phi-luong?key={key}");
                SalaryRequestDocument = new SalaryExpenseAccountingModel();
                ListSalaryExpenseAccounting = new List<SalaryExpenseAccounting1Model>();
                pActionType = nameof(EnumType.Add);
                pDocEntry = -1;
        ...
        protected void GridEditSavingHandler(GridEditModelSavingEventArgs e)
        {
            try
            {
                var itemEdit = (SalaryExpenseAccounting1Model)e.EditModel;
                var itemFind = ListSalaryExpenseAccounting?.FirstOrDefault(m => m.salaryCatagoryCode == itemEdit.salaryCatagoryCode && m.lineId == itemEdit.lineId && m.id == itemEdit.id);
                if (itemFind == null) return;
                itemFind.account1 = itemEdit.account1;
                itemFind.account2 = itemEdit.account2;
                StateHasChanged();
            }
            catch (Exception ex)
            {
