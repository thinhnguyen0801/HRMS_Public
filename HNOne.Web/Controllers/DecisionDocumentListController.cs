...
{
    public class DecisionDocumentListController : DocumentControllerBase
    {
        ...
        #region Properties
        public List<DecisionDocumentModel>? ListPending { get; set; }
        public IGrid? GridPending { get; set; }
        public List<DecisionDocumentModel>? ListAll { get; set; }
            ...
            if (firstRender)
            {
                try
                {
                    ...
                    ListBreadcrumbs = new List<BreadcrumbModel>()
                    {
                        ...
                        new BreadcrumbModel("Danh sách chứng từ quyết định", isActive: true)
                    };
        ...
        private async Task getDecisionDocumentList()
        {
            ...
            var listResult = await _workforceService.GetDecisionDocumentAsync(request, isShowToast: true);
        ...
        protected async Task RedirectPageDetailHandler()
        {
            try
            {
                ...
                _navigationManager.NavigateTo($"/chung-tu-quyet-dinh?key={key}");
            }
            catch { }
        }
        ...
        protected async Task ExportExcelHandler()
        {
            try
            {
                if (ActiveTabIndex == 0)
                {
                    ...
                    await ShowLoading();
                    await GridPending!.ExportToXlsxAsync("Danh-chung-tu-quyet-dinh", new GridXlExportOptions()
                    {
                ...
                await ShowLoading();
                await GridAll!.ExportToXlsxAsync("Danh-chung-tu-quyet-dinh", new GridXlExportOptions()
                {
                    ExportTotalSummaries = false,
                    ExportGroupSummaries = false
                });
